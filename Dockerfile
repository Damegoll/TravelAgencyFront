...
# We're using a lightweight Node.js image (version 20) based on Alpine Linux for efficiency.
...
# STEP 9.5: Fallback all the unknown routes to index.html
# This is still kinda confusing but because of how this file is built, if we ever try to recharge a page
# it will die because it cant find it, so we let React Router take over and handle the route correctly
